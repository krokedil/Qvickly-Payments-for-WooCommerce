...
class Session {
	...
	public const SESSION_KEY = '_qvickly_payments_session_data';
	...
	public function get_session( $order = false ) {
		...
		if ( ! isset( $gateways['qvickly_payments'] ) ) {
			return;
		}
		...
		$result = Qvickly_Payments()->api()->create_session();
		return $this->process_result( $result, $order );
	}
	...
	private function get_order( $order ) {
		...
		return empty( $order ) ? new \WP_Error( 'qvickly_payments_order_not_found', __( 'Order not found', 'qvickly-payments-for-woocommerce' ) ) : $order;
	}
