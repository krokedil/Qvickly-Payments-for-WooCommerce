...
class Plugin {
	...
	public function init() {
		...
		$this->gateway = WC()->payment_gateways->payment_gateways()['qvickly_payments'];
	}
	...
	private function setup_hooks() {
		 ...
		 *
		 * In the templates/payment-categories.php file, we insert the payment categories as unique payment methods (gateways), each with a distinct ID. When Woo process the payment, it will look for a gateway with those IDs, but they don't exist. Only 'qvickly_payments' actually exists. For this reason, we must override the payment method ID to 'qvickly_payments' when the checkout data is posted.
		 */
		add_filter(
			'woocommerce_checkout_posted_data',
			function ( $data ) {
				if ( false !== strpos( $data['payment_method'], 'qvickly_payments' ) ) {
					$data['payment_method'] = 'qvickly_payments';
				}
	...
	public function plugin_action_links( $links ) {
		...
		$plugin_links  = array(
			'<a href="' . $settings_link . '">' . __( 'Settings', 'qvickly-payments-for-woocommerce' ) . '</a>',
		);
